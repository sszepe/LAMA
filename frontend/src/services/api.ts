...
export async function getEntities(params: GetEntitiesParams): Promise<{
  entities: EntityResp[];
  totalCount: number;
  firstIndex: number;
  lastIndex: number;
...
export async function getGraphDataClip(clipId: string): Promise<GraphData> {
  return request('GET', `graph/clip/${clipId}`);
}

export async function getGraphDataEntity(entityId: string): Promise<GraphData> {
  return request('GET', `graph/entity/${entityId}`);
}
