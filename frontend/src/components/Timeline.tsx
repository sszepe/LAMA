...
import { ClipElement, Segment } from '../types/clips';
...
const TRACKLABELS = ['Segment', 'Music', 'Speech', 'Other Sounds', 'Picture'];
...
export const Timeline: React.FC<EmptyObject> = () => {
  ...
  const baseTracks = [
    ...
    clip.elements.Music.map(getE),
    clip.elements.Speech.map(getE),
    clip.elements.Noise.map(getE),
    clip.elements.Picture.map(getE),
  ].map(elements =>
    elements.filter(
      (t: ClipElement) => t.timecodes !== undefined && t.timecodes.length > 0,
    ),
  );
  const tracks = baseTracks.filter((track: ClipElement[]) => track.length > 0);
              ...
              element.timecodes.map(([start, end], k) => (
                <LightTooltip
                  ...
                  title={
                    <div>
                      {element.label} (
                      {element.type === 'Noise' ? 'Other Sounds' : element.type}
                      ){' '}
