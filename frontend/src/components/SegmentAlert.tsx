...
export const SegmentAlert: FC<EmptyObject> = () => {
  ...
  return (
    <Alert
      id="LamaSegmentAlert"
      title="Go to active Segment"
      style={{ cursor: 'pointer' }}
      onClick={e => {
        e.preventDefault();
        dispatch(doJumpTo(segment._id, ['Segment', segment._id]));
      }}
    ...
    >
      <span>
        Segment <strong>{segment.label}</strong> is active. Annotations you make
        will be assigned to this segment.
      </span>
    </Alert>
  );
};
