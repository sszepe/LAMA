...
from lama.database import db, get_document_by_id, get_mongo_data, init_db, ASC, DESC
...
from lama.query import get_clips_containing_entities
...
def get_the_clips():
    ...
    if text_match_q:
        ...
        result = [c for c in clips if c["_id"] in matching_clip_ids]
    else:
        result = clips
    # pagination
    (
...
ENTITIES_SORT_FIELDS = ["label", "type", "createdBy", "updated", "usageCount"]
...
def get_the_entities():
    ...
    entities = get_entities(
        filter_cats=cats, filter_types=types, sort_by=sort_by
    )
    # text filter
    text_match_q = request.query.getunicode("match")
    if text_match_q:
...
def do_graph_data_clip(clip_id):
    ...
    """
    return get_graph_data_clip(clip_id)
...
def do_graph_data_entity(entity_id):
    ...
    """
    return get_graph_data_entity(entity_id)
...
def get_git_revision():
    ...
    if revision_from_environment is not None:
        ...
        return
    try:
        with open("git_revision.txt") as f:
...
# AUTH_TOKEN_EXPIRES_IN = 15  # seconds
...
def _set_cookie(key, value):
    ...
    response.set_cookie(
        ...
        httponly=True,
        secure=GIT_REVISION != "dev" and not is_testing_locally,
